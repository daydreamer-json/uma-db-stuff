...
async function parseCsvFile(filePath: string) {
  const inputContext = (await fs.promises.readFile(path.resolve(filePath), { encoding: 'utf-8' }))
    .replaceAll('\r\n', '\n')
    .trim();
...
function parseJsonMusicscoreLyrics(inputObj: Array<TypesAssetCsvStructure.MusicscoreLyricsOrig>): {
  ...
  return {
    ...
    lrcEncoded: inputObj
      .map((entry) => {
        return `[${`${Duration.fromMillis(parseInt(entry.time)).toFormat('mm:ss')}.${String(Math.round(parseInt(entry.time) / 10) % 100).padStart(2, '0')}`}]${entry.lyrics}`;
      })
      .join('\n'),
  };
}
